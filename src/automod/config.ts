...
// Filter identifiers — also used as `filter_kind` in panda.automod_offenses.
...
function rowToConfig(row: ConfigRow): AutomodConfig {
  return {
...
const cache = new Map<string, { config: AutomodConfig; expiresAt: number }>();
...
export async function getAutomodConfig(serverId: string): Promise<AutomodConfig> {
  ...
  const res = await pool.query<ConfigRow>(
    `SELECT ${SELECT_COLUMNS} FROM panda.automod_config WHERE server_id = $1`,
    [serverId],
  );

  const config = res.rows[0] ? rowToConfig(res.rows[0]) : DEFAULTS(serverId);
...
export async function setAutomodConfig(
  ...
  const res = await pool.query<ConfigRow>(
    `INSERT INTO panda.automod_config (server_id, ${cols.join(', ')})
...
export function invalidateAutomodConfig(serverId: string): void {
